import { getEdgeLabel } from "@/utils/rawGraphUtils";

export class GraphAbstractor {
  private nodes: any[];
  private edges: any[];
  ...
  public abstraction(): { nodes: any[], edges: any[] }{
    ...
    const typeMap = this.buildEdgeMapType2(edgesMap.get("type"));
  ...
  private buildEdgeMapType2(edges: Set<any> | undefined): Map<string, Set<string>> {
    const map = new Map<string, Set<string>>();
    edges?.forEach(edge => {
